...
test('control-plane image and deploy path pin and verify the production supply chain', () => {
  ...
  assert.match(dockerfile, /ARG KUBECTL_VERSION=v1\.34\.11/)
  assert.match(dockerfile, /ARG HELM_VERSION=v4\.2\.4/)
  assert.equal((dockerfile.match(/sha256sum --check --strict/g) || []).length, 2)
  assert.match(dockerfile, /FROM node-tools AS tools/)
  assert.equal((dockerfile.match(/go mod verify/g) || []).length, 2)
